...
export class CreditNoteService {
  ...
  async recalculateTotals(
    ...
    // Balance = total - allocated (reserved) - applied (finalized)
    const balance =
      total -
    ...
    creditNote.balance = Math.max(0, balance);
  ...
  async apply({
    ...
    // Balance = total - allocated - applied
    const newBalance =
    ...
    // If fully applied (no balance and no pending allocations), change status
    if (newBalance <= 0 && Number(creditNote.allocatedAmount) === 0) {

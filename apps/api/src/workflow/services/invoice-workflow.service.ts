...
export interface InvoiceWorkflowResult {
  ...
  creditAllocationsApplied?: number; // Amount of credit allocations finalized
  error?: string;
}
...
export class InvoiceWorkflowService {
   ...
   * 1. Finalize any pending credit allocations for this invoice
   * 2. Record NET receivable amount (total - credit allocations) to Blnk
   * 3. Update invoice status
   */
  async send(input: InvoiceWorkflowInput): Promise<InvoiceWorkflowResult> {
    // Step 1: Finalize any pending credit allocations
    let creditAllocationsApplied = 0;
    try {
      const allocationResult =
        await this.creditAllocationService.finalizeAllByTarget({
          companyId: input.companyId,
          targetType: 'invoice',
          targetId: input.invoiceId,
        });
      creditAllocationsApplied = allocationResult.totalAmount;

      if (creditAllocationsApplied > 0) {
        this.logger.log(
          `Finalized ${creditAllocationsApplied} in credit allocations for invoice ${input.invoiceId}`,
        );
      }
    } catch (error) {
      this.logger.error(`Failed to finalize credit allocations: ${error}`);
      return {
        success: false,
        finalState: 'draft',
        dbStatus: 'draft',
        error: `Failed to finalize credit allocations: ${error instanceof Error ? error.message : String(error)}`,
      };
    }

    // Step 2: Build context with NET amount for Blnk
    // The receivable recorded should be: total - creditAllocationsApplied
    const netTotal = input.total - creditAllocationsApplied;
    const context = this.buildContext({
      ...input,
      // Pass net total for Blnk recording
      total: netTotal,
      // Update balance to reflect credits applied
      balance: input.balance - creditAllocationsApplied,
    });

    // If net total is 0 or negative (fully covered by credits), skip Blnk
    if (netTotal <= 0) {
      this.logger.log(
        `Invoice ${input.invoiceId} fully covered by credit allocations, skipping Blnk`,
      );

      // Update invoice directly to 'sent' (or 'paid' if fully covered)
      const newStatus = netTotal <= 0 ? 'paid' : 'sent';
      await this.invoiceRepository.update(input.invoiceId, {
        status: newStatus as InvoiceEntity['status'],
        amountPaid: creditAllocationsApplied,
        balance: Math.max(0, input.total - creditAllocationsApplied),
      });

      return {
        success: true,
        finalState: newStatus,
        dbStatus: newStatus,
        creditAllocationsApplied,
      };
    }
    ...
    // Step 4: Update the invoice with final status and credit info
   ...
   *
   * Also releases any pending credit allocations back to the credit notes.
   */
  async cancel(
    ...
    try {
      // Release any pending credit allocations
      ...
      if (releasedAllocations.length > 0) {
        ...
        this.logger.log(
          `Released ${totalReleased} in credit allocations for cancelled invoice ${input.invoiceId}`,
        );
      }
